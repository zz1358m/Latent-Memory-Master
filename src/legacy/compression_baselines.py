...
class LLMLinguaBaseline:
    ...
    def answer(self, question: str, sentences: List[str]) -> Tuple[str, int]:
        ...
        self._last_no_system_tokens = _count_tokens_from_marker(
    ...
    def run(self, samples: List[Dict], desc: str = "LLMLingua") -> Dict[str, float]:
        predictions, references, token_counts, token_counts_no_system = [], [], [], []
        ...
        for sample in tqdm(samples, desc=desc):
            ...
            token_counts_no_system.append(self._last_no_system_tokens)

        if not predictions:
            return {"em": 0.0, "f1": 0.0, "rouge_l": 0.0, "avg_tokens": 0.0,
                    "recall_at_k": 0.0, "precision_at_k": 0.0}
        metrics = evaluate(predictions, references, token_counts, token_counts_no_system=token_counts_no_system)
...
class AutoCompressorBaseline:
    ...
    For long contexts, the official implementation also supports implicit
    multi-step compression via `segment_lengths`, which we use here to compress
    the entire context instead of pre-filtering sentences with retrieval.
    ...
    def __init__(
        ...
        self.max_new_tokens  = max_new_tokens
        self.device          = device
        # Kept for caller compatibility; AutoCompressor itself does not use top-k retrieval.
        self.top_k           = top_k
    ...
    def _get_context(self, sentences: List[str]) -> str:
        return " ".join(sentences)

    def _segment_lengths(self, total_tokens: int) -> List[int]:
        if total_tokens <= 0:
            return []
        full_segments, remainder = divmod(total_tokens, self.segment_length)
        lengths = [self.segment_length] * full_segments
        if remainder:
            lengths.append(remainder)
        return lengths

    def _compress_context(self, context: str) -> Tuple[torch.Tensor, int]:
        ctx_ids = self.tokenizer(
            context,
            add_special_tokens=False,
            return_tensors="pt",
        ).input_ids.to(self.device)
        segment_lengths = self._segment_lengths(ctx_ids.shape[1])
        if not segment_lengths:
            raise ValueError("Context tokenization produced an empty sequence.")
        with torch.no_grad():
            summary_vectors = self.model(
                ctx_ids,
                segment_lengths=segment_lengths,
                output_softprompt=True,
            ).softprompt
        return summary_vectors, ctx_ids.shape[1]

    def answer(self, question: str, sentences: List[str]) -> Tuple[str, int]:
        if not sentences:
            return "", 0

        context = self._get_context(sentences)

        try:
            summary_vectors, ctx_len = self._compress_context(context)
            ...
            return answer, ctx_len + prompt_ids.shape[1]
    ...
    def answer_batch(self, questions: List[str], sentences_list: List[List[str]]) -> List[Tuple[str, int]]:
        ...
        try:
            ...
            for q, sentences in zip(questions, sentences_list):
                context = self._get_context(sentences)
                sv, ctx_len = self._compress_context(context)
    ...
    def run(self, samples: List[Dict], batch_size: int = 4) -> Dict[str, float]:
        ...
        for i in tqdm(range(0, len(valid), batch_size), desc="AutoCompressor"):
            ...
            for s, (ans, n_tok) in zip(batch, ans_list):
                ...
                token_counts.append(n_tok)
                token_counts_no_system.append(n_tok)
...
class XRAGBaseline:
    ...
    def answer(self, question: str, sentences: List[str],
        ...
        try:
            ...
            if (
                generated_ids.shape[0] > input_len
                and torch.equal(generated_ids[:input_len], input_ids[0])
            ):
                generated_ids = generated_ids[input_len:]
            answer = self.xrag_tokenizer.decode(generated_ids, skip_special_tokens=True).strip()
            n_from_background = _count_tokens_from_marker(
                self.xrag_tokenizer, prompt, ["Background:"]
            )
            return answer, n_from_background
